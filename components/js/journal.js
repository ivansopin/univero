var Journal = Base.extend({
	...
	generateEntry: function() {
		...
		this.journal = this.journal.replace(/&/g, "&amp;");
		
		spacePos = this.journal.indexOf(" ");
		
		if (spacePos > -1) {
			this.journal = this.journal.substring(0, spacePos) + "&nbsp;" + this.journal.substring(spacePos + 1);
		}
